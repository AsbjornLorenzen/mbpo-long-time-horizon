...
def batched_k_combinations_indices(tensor_batch, k):
    batch_combinations = []
    
    for i, tensor in tqdm(enumerate(tensor_batch)):
        num_elements = len(tensor)
        k_combs_indices = list(itertools.combinations(range(num_elements), k))  # Get index combinations
        batch_combinations.append(k_combs_indices)

    return batch_combinations  # List of tensors, each containing index combinations
...
def rollout_model_and_populate_sac_buffer(
    ...
    for i in range(max_rollout_length):
        ...
        for combination in model_combinations: 
            ...
            means_of_subset = means_of_all_ensembles[combination, :, :]
            vars_of_subset = vars_of_all_ensembles[combination, :, :]

            candidate_jsp = dm.calc_pairwise_symmetric_uncertainty_for_measure_function(means_of_subset,
                                                                                    vars_of_subset,
                                                                                    3,
            ...
            lowest_jsp = torch.minimum(lowest_jsp, torch.tensor(candidate_jsp))
            
        
        uncertainty_score = lowest_jsp
